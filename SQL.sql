...
WITH cohort as(
    ...
    where Date_Trunc('month', t.transaction_date)::DATE <= cohort_month + INTERVAL '1 year'
), c as( /*for first year*/
    SELECT
        cohort_month,
    ...
    INNER JOIN cohort USING (customer_id)
    where Date_Trunc('month', t.transaction_date)::DATE >= cohort_month + INTERVAL '1 year'
    AND Date_Trunc('month', t.transaction_date)::DATE <= cohort_month + INTERVAL '2 year'
...
******************************************************************************************/
    ...
    /*Build a one-page executive summary: current MRR, MoM growth %, churn rate, NRR, blended CAC, CLTV:CAC ratio. Flag anything that should worry the board.*/
