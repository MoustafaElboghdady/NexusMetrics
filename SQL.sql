...
WITH s as(
    ...
    FROM s
    WHERE transaction_month < '2026-08-01'
...
/*GRR for the fisrt year Compared with GRR for the second Year*/


WITH cohort as(
        SELECT 
            customer_id,
            Date_Trunc('month', signup_date)::DATE as cohort_quarter
        FROM customers
    ...
    where Date_Trunc('month', t.transaction_date)::DATE <= cohort_quarter + INTERVAL '1 year'
), c as( /*for first year*/
    SELECT
        cohort_quarter,
    ...
    from b
    GROUP BY customer_id, cohort_quarter
    ...
    INNER JOIN cohort USING (customer_id)
    where Date_Trunc('month', t.transaction_date)::DATE >= cohort_quarter + INTERVAL '1 year'
    AND Date_Trunc('month', t.transaction_date)::DATE <= cohort_quarter + INTERVAL '2 year'
    ...
    SELECT
        ...
        ROUND((SUM(starting_mrr) - SUM(churn_amount) - SUM(contraction_amount))::NUMERIC / SUM(starting_mrr),2)*100 as GRR
    FROM c
), g as( /*for second year*/
    SELECT
        SUM(starting_mrr) as mrr,
        SUM(churn_amount) as churn,
        SUM(contraction_amount) as contraction,
...
SELECT
    'Year 1' as period,
    f.mrr, f.churn, f.contraction, f.GRR
FROM f
...
SELECT
    'Year 2' as period,
    g.mrr, g.churn, g.contraction, g.GRR
FROM g
...
SELECT
    ...
    ROUND((SUM(starting_mrr) - SUM(churn_amount) - SUM(contraction_amount) + SUM(expanion_amount)):: NUMERIC / SUM(starting_mrr),2)*100 as NRR
FROM c
), g as(
    /*for second year*/
SELECT
...
WITH customer_summary AS (
    SELECT
        ...
        COALESCE(SUM(t.amount) FILTER (WHERE t.amount > 0), 0) AS total_customer_revenue
    FROM customers c
    LEFT JOIN transactions t 
        ON c.customer_id = t.customer_id
    WHERE c.final_status = 'Churned'
    GROUP BY c.customer_id, c.signup_date, c.first_churn_date
)
SELECT
    ROUND(AVG(total_customer_revenue), 2) AS average_clv,
    ROUND(AVG(lifespan_days), 2) AS avg_lifespan_days,
    ROUND(AVG(lifespan_days) / 30.0, 2) AS avg_lifespan_months
...
WITH total as(
    SELECT
        c.customer_id,
        c.acquisition_cost::numeric as acq_cost,
        c.final_status,
        SUM(t.amount) FILTER (WHERE c.final_status = 'Churned') as total_paid_churned,
        CASE 
            WHEN c.final_status = 'Churned' 
            THEN (c.first_churn_date::date - c.signup_date::date)::numeric / 30.0
            ELSE NULL
        END as lifespan_months
    FROM customers c
    LEFT JOIN transactions t ON c.customer_id = t.customer_id
    WHERE t.amount > 0
    GROUP BY c.customer_id, c.acquisition_cost, c.final_status, c.first_churn_date, c.signup_date
),final as(
    SELECT
        ROUND(SUM(acq_cost) / COUNT(DISTINCT customer_id), 2) as avg_cac,
        ROUND(AVG(lifespan_months), 2) as avg_lifespan,
        ROUND(SUM(total_paid_churned) / SUM(lifespan_months), 2) as avg_monthly_purchase,
        ROUND(AVG(lifespan_months) * (SUM(total_paid_churned) / SUM(lifespan_months)), 2) as clv
    FROM total
)
SELECT *,
    ROUND(clv / avg_cac, 2) as clv_cac
FROM final
        ...
        /* for each Cohort, for how long we keep cohort customer's 
                            "Classic Cohort Chart" */


WITH cohort as(
    SELECT 
            Date_Trunc('quarter', c.signup_date)::DATE as cohort_quarter,
            date_trunc('quarter', t.transaction_date::date)::Date as quarter,
            COUNT(DISTINCT c.customer_id) FILTER (
                WHERE t.transaction_type != 'Cancellation'
            ) as count_exist
        FROM customers c LEFT JOIN transactions t 
        ON c.customer_id = t.customer_id
        GROUP BY cohort_quarter,quarter
        ORDER BY cohort_quarter,quarter
), activity as(
    SELECT 
        *,
        FIRST_VALUE(count_exist) OVER(PARTITION BY cohort_quarter
        ORDER BY quarter) as starting_count,
        ROUND((quarter - cohort_quarter)::numeric / 90,0) as quarters_since_singup
    FROM cohort
),final as(
    SELECT
        cohort_quarter,starting_count,count_exist, quarter,quarters_since_singup,
        ROUND((count_exist::numeric / starting_count)*100,2) as retention_rate
    FROM activity 
) SELECT
        *
    FROM final
    WHERE quarters_since_singup = '4'
    AND retention_rate > '80'
